...
Some notes about various utilities (Lots of others probably here on [GTFObins](https://gtfobins.org/)):
 ...
 - `env` Tells us about the environment variables. (which shell, path, current directory, etc.) a `-i` will **i**gnore the current environment variables when executing the next command
 - `setarch` Sets the architecture of a process. Common architectures: `i386`, `x86\_64`, `arm`, `aarch64`  (eg. you have an x86_64 system, but want to run a program as if it was on a 32 bit system, you can do `setarch i386 ./myprogram <arg1> <arg2> ...`
 - `watch <command>` this executes a command continuously (sometimes quotes are needed for it, depeneding on complexity, such as pipes `|`) and watches the output, by default it happens every 2 seconds. Normally the command is passed into `bash -c` so, it's bash that's executing the command. `watch` can do it directly by using the `-x` or `--exec` flag
 - `socat` like `cat` for concatenation and sockets. Connects two data sources together and transfers between them. These data sources could be unix sockets, UDP, TCP, stdin. But everything on linux is a file so you can just transfer content of one file into another `socat /path/to/file1 /path/tofile2` by default takes the content of file1 and appends to file2 `socat -U /path/to/file1 /path/to/file2` reverses the unidirectional transfer from 2nd arg to 1st. Absolute path seems to be required.
 - `whiptail` Very interesting. pop-up dialog box within the terminal which I've seen when changing settings. Won't rehash a tutorial, but there's a great one on [wikibooks](https://en.wikibooks.org/wiki/Bash_Shell_Scripting/Whiptail). A file can be a source of the information for the pop-up (such as titles, message content) `whiptail --textbox <file> 12 80` The last two numbers are the height and width of the screen
 - `cp` This is a very standard command, but an interesting thing about it is that when it makes a new file from the copying, it retains original permissions. But if the file exists and is being overwritten, the original permissions are preserved. creation vs overwriting
